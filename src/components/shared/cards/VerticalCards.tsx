                        ...
                        <div key={anime.id} className=" border-2 border-white/30 card-img rounded-lg">
                            ...
                            <Link href={`/details/${anime.id}`} className="content-normal overflow-hidden w-full h-full">
                                <div className="md:w-48 h-60 w-50 relative overflow-hidden">
                                    <img src={anime?.image} alt={`an image of ${anime?.title?.userPreferred || anime?.title?.english || anime?.title?.romaji || anime.title?.native}`} className="rounded-lg hover:scale-105 duration-200 h-60 lg:h-64 w-full " draggable="false" loading="lazy" height={400} width={200} />
                                </div>
