                    ...
                    props?.map((anime) => {
                        return (
                            <Link href={`details/${anime.id}`} className="flex gap-5 items-center" key={anime.id + 1}>
                                <img src={anime?.image} loading="lazy" alt={`an image of ${anime?.title?.userPreferred || anime?.title?.english || anime?.title?.romaji || anime?.title?.native}`} height={200} width={400} className="lg:w-32 w-28 duration-200 hover:scale-105 rounded-lg" />
