        ...
        <div className={`mb-20 md:mb-0 p-4 mt-12 md:mt-0 border-t border-black/20 dark:border-white/20`}>
            <div className="flex flex-col gap-7 justify-between items-center">
                ...
                <p className="text-[9px] md:text-sm text-center font-semibold m-auto max-w-4xl">
                    AnimeTrix is not affiliated with or endorsed by any of the anime studios behind the creation of the
                    anime presented on this site. This website is only a user interface presenting/linking various
                    self-hosted files across the internet by other third-party providers for easy access. AnimeTrix never
                    downloads the video from any source provider, link will be returned from the response hence it is
                    completely not subjected to DMCA compliant.
                </p>
            </div>
                ...
                <div className="flex justify-between mt-12 items-center">
                    ...
                    <div className="flex gap-4 items-center">
                        {themeOptions.map((opt) => (
                            <button
                                key={opt.name}
                                className={`p-2 rounded-full dark:bg-white/10 bg-black/10`}
                            >
                                {opt.icon}
                            </button>
                        ))}
