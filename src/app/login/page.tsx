        ...
        <section className='flex min-h-[70vh] justify-center items-center'>
            <div className=' bg-black/20 dark:bg-white/20 shadow-md shadow-black/70 dark:shadow-white/70 w-full md:w-1/2 2xl:w-1/3 m-4 md:m-auto p-4 rounded-lg'>
                ...
                <form autoComplete='false' className="flex flex-col gap-2" onSubmit={handleLogin}>
                    ...
                    <input type="email" placeholder='Email' onChange={(e) => setEmail(e.target.value)} className=' bg-transparent border-2 border-black/20 dark:border-white/20 p-2 focus:outline-none  rounded-lg dark:text-white text-black' autoComplete='off' />
                    <label htmlFor="Password">Password</label>
                    <div className="flex justify-between items-center border-2 rounded-lg dark:border-white/20 border-black/20 p-2 ">
                    ...
                    </div>
                    {
                        loading ? <button className=' font-semibold flex gap-3 p-3  bg-white text-black rounded-lg items-center justify-center' disabled={true}>
                            ...
                            <button className={` p-3 ${email && password != "" ? "bg-blue-600 cursor-pointer" : "dark:bg-white/30 bg-black/30 dark:text-black text-white cursor-not-allowed"} rounded-lg mt-3 font-semibold duration-200 ${email && password != "" && "hover:bg-white"} hover:text-black`} disabled={email && password != "" ? false : true}>Login</button>
                        )
                    }
