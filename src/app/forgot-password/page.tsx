                ...
                <form autoComplete='false' className="flex flex-col gap-2" onSubmit={handleResetPassword}>
                    ...
                    <input type="email" placeholder='Email' onChange={(e) => setEmail(e.target.value)} className=' bg-transparent border-2 dark:border-white/20 border-black/20 p-2 focus:outline-none  rounded-lg dark:text-white text-black' autoComplete='off' />
                    {
                        loading ? <button className=' font-semibold flex gap-3 p-3  bg-white text-black rounded-lg items-center justify-center' disabled={true}>
                            ...
                            <button className={` p-3 ${email != "" ? "bg-blue-600 cursor-pointer" : "dark:bg-white/30 bg-black/30 dark:text-black text-white cursor-not-allowed"} rounded-lg mt-3 font-semibold duration-200 ${email != "" && "hover:bg-white"} hover:text-black`} disabled={email != "" ? false : true}>Send Email</button>
                        )
                    }
                </form>
            </div>
